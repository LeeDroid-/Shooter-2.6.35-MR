	...
	if (data->payload_size) {
		...
		case ADM_CMDRSP_COPP_OPEN: {
			...
			if (open->copp_id == INVALID_COPP_ID) {
				pr_aud_err("%s: invalid coppid rxed %d\n",
...
void send_cal(int port_id, struct acdb_cal_block *aud_cal)
{
	...
	pr_aud_info("%s: Sending SET_PARAMS payload = 0x%x, size = %d\n",
...
void send_adm_cal(int port_id, int path)
{
	...
	pr_aud_info("%s: Sending audproc cal, acdb_path %d\n",
		    __func__, acdb_path);
	get_audproc_cal(acdb_path, &aud_cal);
	send_cal(port_id, &aud_cal);

	pr_aud_info("%s: Sending audvol cal, acdb_path %d\n",
		    __func__, acdb_path);
...
int adm_open(int port_id, int path, int rate, int channel_mode, int topology)
{
	...
	pr_aud_info("Topology = %x\n", topology);

	if (afe_validate_port(port_id) < 0) {
		pr_aud_err("%s port idi[%d] is invalid\n", __func__, port_id);
		return -ENODEV;
	}
			...
			unsigned int *port_id, int copp_id)
{
	...
	for (i = 0; i < num_copps; i++) {
		...
		if (tmp < 0 || tmp >= AFE_MAX_PORTS) {
			pr_aud_err("afe_get_port_index return invalid %d\n", tmp);
			ret = -EINVAL;
			goto fail_cmd;
		}
				...
				uint32_t *bufsz, uint32_t bufcnt)
{
	...
	if (!mmap_region_cmd) {
		pr_aud_err("%s: allocate mmap_region_cmd failed\n", __func__);
		return -ENOMEM;
	}
						...
						uint32_t bufcnt)
{
	...
	if (!unmap_region_cmd) {
		pr_aud_err("%s: allocate unmap_region_cmd failed\n", __func__);
		return -ENOMEM;
	}
...
int adm_get_copp_id(int port_id)
{
	...
	if (port_id < 0) {
		pr_aud_err("%s: invalid port_id = %d\n", __func__, port_id);
		return -EINVAL;
	}
...
int adm_close(int port_id)
{
	...
	pr_aud_info("%s port_id=%d index %d\n", __func__, port_id, index);
