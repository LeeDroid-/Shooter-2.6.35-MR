...
void rtac_add_dev_ctrl_device(u32 dev_id, struct msm_snddev_info *dev_info)
{
	...
	if (rtac_dev_ctl_data.num_of_dev == RTAC_MAX_ACTIVE_DEVICES) {
		pr_aud_err("%s, Can't add anymore RTAC devices!\n", __func__);
		goto done;
	}
...
void rtac_copy_adm_payload_to_user(void *payload, u32 payload_size)
{
	...
	if (payload_size != 0) {
		if (payload_size > rtac_adm_user_buf_size) {
			pr_aud_err("%s: Buffer set not big enough for "
...
u32 send_adm_apr(void *buf, u32 opcode)
{
	...
	if (copy_from_user(&count, (void *)buf, sizeof(count))) {
			pr_aud_err("%s: Copy to user failed! buf = 0x%x\n",
	...
	if (count <= 0) {
		pr_aud_err("%s: Invalid buffer size = %d\n", __func__, count);
		goto done;
	}

	if (copy_from_user(&payload_size, buf + sizeof(u32), sizeof(u32))) {
		pr_aud_err("%s: Could not copy payload size from user buffer\n",
			__func__);
		goto done;
	}
	...
	if (payload_size > MAX_PAYLOAD_SIZE) {
			...
			pr_aud_err("%s: Invalid payload size = %d\n",
				__func__, payload_size);
		goto done;
	}

	if (copy_from_user(&copp_id, buf + 2 * sizeof(u32), sizeof(u32))) {
		pr_aud_err("%s: Could not copy port id from user buffer\n",
			__func__);
		goto done;
	}
	...
	if (port_id >= AFE_MAX_PORTS) {
		pr_aud_err("%s: Invalid Port ID = %d\n", __func__, port_id);
		goto done;
	}

	mutex_lock(&rtac_adm_apr_mutex);
	if (rtac_adm_apr_data.apr_handle == NULL) {
		pr_aud_err("%s: APR not initialized\n", __func__);
		goto err;
	}
	...
	if (copy_from_user(rtac_adm_buffer + sizeof(adm_params),
		...
		pr_aud_err("%s: Could not copy payload from user buffer\n",
			__func__);
		goto err;
	}
	...
	if (result < 0) {
		pr_aud_err("%s: Set params failed port = %d\n",
			__func__, port_id);
		goto err;
	}
	...
	mutex_unlock(&rtac_adm_apr_mutex);
	if (!result) {
		pr_aud_err("%s: Set params timed out port = %d\n",
			__func__, port_id);
		goto done;
	}
	...
	if (rtac_adm_payload_size != 0) {
		if (copy_to_user(buf, rtac_adm_buffer,
			...
			pr_aud_err("%s: Could not copy buffer to user,"
				"size = %d\n", __func__, payload_size);
			goto done;
		}
	}
...
u32 send_rtac_asm_apr(void *buf, u32 opcode)
{
	...
	if (copy_from_user(&session_id, buf + 2 * sizeof(u32), sizeof(u32))) {
		pr_aud_err("%s: Could not copy session id from user buffer\n",
			__func__);
		goto done;
	}
	if (session_id > SESSION_MAX) {
		pr_aud_err("%s: Invalid Session = %d\n", __func__, session_id);
		goto done;
	}
	...
	if (result < 0) {
		pr_aud_err("%s: Set params failed session = %d\n",
			__func__, session_id);
		goto err;
	}
	...
	mutex_unlock(&rtac_asm_apr_mutex);
	if (!result) {
		pr_aud_err("%s: Set params timed out session = %d\n",
			__func__, session_id);
		goto done;
	}
...
u32 send_voice_apr(u32 mode, void *buf, u32 opcode)
{
	...
	if ((mode != RTAC_CVP) && (mode != RTAC_CVS)) {
		pr_aud_err("%s: Invalid Mode for APR, mode = %d\n",
			__func__, mode);
		goto done;
	}
	...
	if (result < 0) {
		pr_aud_err("%s: apr_send_pkt failed opcode = %x\n",
			__func__, opcode);
		goto err;
	}
	/* Wait for the callback */
	...
	mutex_unlock(&rtac_voice_apr_mutex);
	if (!result) {
		pr_aud_err("%s: apr_send_pkt timed out opcode = %x\n",
			__func__, opcode);
		goto done;
	}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	if (arg == 0) {
		pr_aud_err("%s: No data sent to driver!\n", __func__);
		result = -EFAULT;
		goto done;
	}
	...
	case AUDIO_GET_RTAC_DEV_CTRL_INFO:
		if (copy_to_user((void *)arg, &rtac_dev_ctl_data,
			...
			pr_aud_err("%s: Could not copy to userspace!\n", __func__);
		else
	...
	case AUDIO_SET_RTAC_CVP_CAL:
		...
		break;
	default:
		pr_aud_err("%s: Invalid IOCTL, command = %d!\n",
		       __func__, cmd);
	}
done:
	return result;
...
static int __init rtac_init(void)
{
	...
	if (rtac_adm_buffer == NULL) {
		pr_aud_err("%s: Could not allocate payload of size = %d\n",
