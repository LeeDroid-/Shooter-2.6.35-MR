...
void load_q6(void)
{
	...
	if (q6.state == APR_Q6_NOIMG) {
		q6.pil = pil_get("q6");
		if (!q6.pil) {
			pr_aud_info("APR: Unable to load q6 image\n");
			goto q6_unlock;
		}
		q6.state = APR_Q6_LOADING;
	}
	pr_aud_info("Q6 loading done: Waiting for apr_init\n");
	mutex_lock(&client[dest_id][client_id].svc[svc_idx].m_lock);
	do {
		client[dest_id][client_id].handle = apr_tal_open(client_id,
				dest_id, APR_DL_SMD, apr_cb_func, NULL);
		if (!client[dest_id][client_id].handle) {
			if (q6.state == APR_Q6_LOADED) {
				pr_aud_info("APR: Unable to open handle\n");
				goto unlock;
			}
			udelay(5);
			if (delay_cnt++ < 400000)
			...
			q6.state = APR_Q6_LOADED;
			pr_aud_info("apr_init done\n");
			msleep(50);
			pr_aud_info("Audio init done\n");
		}
		break;
	} while (1);

	if (delay_cnt >= 400000)
		pr_aud_info("Q6 Init not yet done in 2 secs\n");
unlock:
	mutex_unlock(&client[dest_id][client_id].svc[svc_idx].m_lock);
q6_unlock:
	mutex_unlock(&q6.lock);
